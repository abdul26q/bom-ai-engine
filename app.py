...
def get_api_key():
    try:
        return st.secrets.get("DEEPSEEK_API_KEY") or os.environ.get(
            "DEEPSEEK_API_KEY", ""
        )
    except Exception:
        return ""
...
with st.sidebar:
    ...
    st.success("⚡ TraceGuard AI Core: Active (DeepSeek Engine)")
...
# 3. Master DeepSeek AI Core Engine
def analyze_components_with_deepseek(bom_data_str, api_key):
    try:
        # Initialize standard OpenAI client configured for DeepSeek's base URL
        ...
        ====================================================================================================
        Respond STRICTLY in a JSON array format matching this exact structure:
        [
          {{
            "mpn": "MAX232CPE",
        ...
        Output pure JSON only. Do not add markdown codeblocks.
        """
        ...
        # Isolate JSON array from response
...
with tab1:
    ...
    if btn_search and search_mpn.strip():
        ...
        with st.spinner(
            ...
            search_results = analyze_components_with_deepseek(
        ...
        if bom_df is not None and not bom_df.empty:
            ...
            if run_audit:
                ...
                with st.spinner(
                    ...
                    bom_data_str = "\n".join(bom_summary)
                    results = analyze_components_with_deepseek(
